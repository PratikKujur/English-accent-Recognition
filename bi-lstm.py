...
class BiLSTMClassifier(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers, num_classes):
        ...
        self.fc = nn.Linear(hidden_dim * 2, num_classes)  # hidden_dim * 2 for bidirectional

    def forward(self, x):
        # x: [batch, seq_len, input_dim]
        _, (h_n, _) = self.lstm(x)
        h_n = torch.cat((h_n[-2], h_n[-1]), dim=1)  # Concatenate the outputs from both directions
        out = self.fc(h_n)  # Apply fully connected layer
        return out
...
def main():
    ...
    model = BiLSTMClassifier(input_dim=input_dim, hidden_dim=hidden_dim, num_layers=num_layers, num_classes=num_classes)
    model.to(device)
